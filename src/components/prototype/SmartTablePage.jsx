...
function SmartTablePage({
  ...
  const filteredRows = useMemo(() => {
    ...
    const found = query
      ? rows.filter((row) => searchFields.some((field) => String(row[field] ?? "").toLowerCase().includes(query)))
      : rows;
    ...
    return [...found].sort((a, b) => {
      ...
      const result = typeof left === "number" && typeof right === "number"
        ? left - right
        : String(left ?? "").localeCompare(String(right ?? ""), "uz");
      return sort.direction === "desc" ? -result : result;
    });
  }, [rows, search, searchFields, sort]);
  ...
  const exportCsv = (sourceRows = filteredRows) => {
    ...
    const body = sourceRows.map((row) => exportColumns.map((column) => csvEscape(row[column.key])).join(",")).join("\n");
  ...
  const problemCount = rows.filter((row) => problemStatuses.includes(row.status)).length;
  const currentCount = rows.filter((row) => Object.hasOwn(row, "status") && !problemStatuses.includes(row.status)).length;
  const hasStatus = rows.some((row) => Object.hasOwn(row, "status"));
  const defaultSummary = [
    { label: "Jami", value: rows.length, hint: "Barcha yozuvlar" },
    ...(hasStatus ? [
            ...
            <div className="qp-drawer-details">
              {visibleColumns.filter((column) => column.label).map((column) => (
                <div className="qp-drawer-detail-row" key={column.key}>
                  <span>{column.label}</span>
                  <div>{column.render ? column.render(selectedRow) : selectedRow[column.key] ?? "—"}</div>
                </div>
              ))}
            </div>
          )
