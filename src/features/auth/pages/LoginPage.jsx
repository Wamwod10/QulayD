...
function LoginPage() {
  ...
  const submit = async ({ identifier, password, pin, mode }) => {
    ...
    try {
      const { user } = mode === "pin" ? await auth.pinLogin(identifier, pin) : await auth.login(identifier, password);
      resetLocalDbCache();
  ...
  return <div className="qp-auth-card">
    ...
    <div className="qp-auth-footer"><Link to="/forgot-password">Parolni unutdingizmi?</Link> · Yangi biznesmisiz? <Link to="/register">Kompaniya yaratish</Link></div>
  </div>;
}
