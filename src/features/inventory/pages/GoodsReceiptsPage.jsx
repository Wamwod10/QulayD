...
function GoodsReceiptsPage() {
  ...
  const submit = (event) => { event.preventDefault(); if (!form.supplierId || !form.warehouseId || !form.productId || Number(form.quantity) <= 0) return; createGoodsReceipt({ supplierId: form.supplierId, warehouseId: form.warehouseId, items: [{ productId: form.productId, quantity: form.quantity, cost: form.cost }] }); setOpen(false); setForm({ supplierId: "", warehouseId: db.warehouses[0]?.id || "", productId: "", quantity: "", cost: "" }); };
