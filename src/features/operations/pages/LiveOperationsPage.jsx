      ...
      <div className="qp-operations-split">
        ...
        <aside className={`qp-card qp-operations-detail ${selectedId ? "is-mobile-open" : ""}`}>
          {selected ? <>
            ...
            {selected.order.approvalStatus === "PENDING" ? <div className="qp-operations-approval-actions">{isOwner ? <><SecondaryButton type="button" onClick={async () => { const result = await rejectOrderRequest(selected.order.id); notify(result.message, result.ok ? "warning" : "danger"); }}>Rad etish</SecondaryButton><PrimaryButton type="button" onClick={async () => { const result = await approveOrderRequest(selected.order.id); notify(result.message, result.ok ? "success" : "warning"); }}>Tasdiqlash <ArrowRight size={15}/></PrimaryButton></> : <span>Owner qarori kutilmoqda</span>}</div> : <button type="button" className="qp-button qp-button-primary qp-operations-open" onClick={() => navigate("/orders")}>Buyurtmani boshqarish <ArrowRight size={15} /></button>}
