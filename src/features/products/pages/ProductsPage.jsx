          ...
          <main className="qp-product-builder-main">
            ...
            <section className="qp-product-form-card">
              ...
              {form.features.variants ? <div className="qp-advanced-editor qp-variant-editor"><div className="qp-editor-head"><div><strong><Layers3 size={16}/> Variant konstruktori</strong><span>Rang, xotira, razmer, ta’m kabi xususiyatlarni kiriting. QULAY kombinatsiyalarni o‘zi yaratadi.</span></div><SecondaryButton type="button" onClick={() => patchForm({ axes:[...form.axes,{name:'',values:''}] })}><Plus size={14}/> Xususiyat</SecondaryButton></div>
                ...
                {form.variants.length ? <div className="qp-variant-table"><div className="qp-variant-row head"><span>Variant</span><span>SKU</span><span>Barcode</span><span>Maxsus narx</span></div>{form.variants.map((item,index) => <div className="qp-variant-row" key={item.id || `${item.name}-${index}`}><strong>{item.name}</strong><input className="qp-input" value={item.sku} onChange={(event)=>updateList('variants',index,{sku:event.target.value.replace(/\D/g,'').slice(0,5)})}/><input className="qp-input" value={item.barcode} placeholder="Ixtiyoriy" onChange={(event)=>updateList('variants',index,{barcode:event.target.value})}/><input className="qp-input" type="number" min="0" value={item.price} placeholder="Asosiy narx" onChange={(event)=>updateList('variants',index,{price:event.target.value})}/></div>)}</div> : null}
              </div> : null}
            </section>
