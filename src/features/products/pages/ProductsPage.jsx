...
function ProductsPage() {
  ...
  const applyPreset = (preset) => setForm((current) => {
    // Tez rejim savdo/tracking modelini almashtiradi, lekin supplier, ombor joyi va markirovka kabi
    // mustaqil biznes sozlamalarini tasodifan o‘chirib yubormaydi.
    const next = { ...current.features, packaging: false, variants: false, expiry: false, lot: false, serial: false };
  ...
  return <>
    <SmartTablePage title="Mahsulotlar" eyebrow="Ombor" description="Savdo, kassa va ombor uchun yagona universal mahsulot katalogi." rows={rows}
      ...
      extraSummary={[{ label: "Faol mahsulot", value: rows.filter((row) => row.status === "ACTIVE").length, hint: "Sotuv uchun faol" },
        ...
        { label: "Variantli", value: rows.filter((row) => row.variants?.length).length, hint: "Variant kombinatsiyalari" }, { label: "Qadoqli", value: rows.filter((row) => row.packages?.length).length, hint: "Asosiy birlik konversiyasi" }]}
      ...
      detailRenderer={(row) => <div className="qp-product-detail"><div className="qp-product-detail-tabs">{detailTabs(row).map((tab) => <button key={tab.id} type="button" className={detailTab === tab.id ? "active" : ""} onClick={() => setDetailTab(tab.id)}>{tab.label}</button>)}</div>
        ...
        {detailTab === "prices" ? <div className="qp-detail-kpis"><div><span>Tannarx</span><strong>{formatMoney(row.costPrice)}</strong></div>{(row.prices || []).map((entry) => <div key={entry.id || entry.priceListId}><span>{entry.priceList?.name || db.priceLists.find((list) => list.id === entry.priceListId)?.name || "Narx"}</span><strong>{formatMoney(entry.price)}</strong></div>)}</div> : null}
      ...
      <form className="qp-smart-product-form qp-product-builder" onSubmit={submit} aria-busy={busy}>
        <div className="qp-product-preset-bar">
          <div>
            ...
            <strong>Mahsulot modeli</strong>
            <small>Tayyor shablonni tanlang yoki pastdagi imkoniyatlarni alohida yoqing.</small>
          </div>
          <div className="qp-product-presets">
            <button type="button" onClick={() => applyPreset("SIMPLE")} className={!form.features.packaging && !form.features.variants && !form.features.serial && !form.features.lot && !form.features.expiry ? "active" : ""}><Package size={16}/><span><b>Oddiy</b><small>Dona / kg / litr</small></span></button>
            ...
            <button type="button" onClick={() => applyPreset("SERIAL")} className={form.features.serial ? "active" : ""}><Barcode size={16}/><span><b>Serial / IMEI</b><small>Har dona alohida</small></span></button>
            <button type="button" onClick={() => applyPreset("LOT")} className={form.features.lot || form.features.expiry ? "active" : ""}><CheckCircle2 size={16}/><span><b>Partiya / muddat</b><small>Lot va yaroqlilik</small></span></button>
          </div>
        </div>
        ...
        <div className="qp-product-builder-layout">
          ...
          <main className="qp-product-builder-main">
            ...
            <section className="qp-product-form-card">
              <header><span className="qp-section-icon"><Layers3 size={17}/></span><div><strong>Savdo va kuzatuv</strong><small>Faqat shu mahsulotga kerak bo‘lgan imkoniyatlarni yoqing</small></div></header>
              ...
              {form.features.packaging ? <div className="qp-advanced-editor qp-packaging-editor"><div className="qp-editor-head"><div><strong><Boxes size={16}/> Qadoqlash birliklari</strong><span>Masalan: 1 pachka = 12 dona, 1 quti = 6 pachka. POS qadoq barcode’ini ham taniydi.</span></div><SecondaryButton type="button" onClick={() => patchForm({ packages: [...form.packages, { name: "", variantSku: "", conversion: "", barcode: "", price: "", costPrice: "" }] })}><Plus size={14}/> Qadoq</SecondaryButton></div>
                ...
                {form.packages.map((item, index) => <div className="qp-package-card" key={item.id || index}><div className="qp-package-card-head"><span><Package size={16}/><b>{item.name || `Qadoq ${index + 1}`}</b>{cleanNumber(item.conversion) > 0 ? <small>1 {item.name || "qadoq"} = {cleanNumber(item.conversion)} asosiy birlik</small> : null}</span><button type="button" className="qp-icon-button" onClick={() => removeList('packages',index)}><Trash2 size={15}/></button></div><div className="qp-package-fields"><Field label="Nomi"><input className="qp-input" placeholder="Pachka / quti" value={item.name} onChange={(event) => updateList('packages',index,{name:event.target.value})}/></Field>{form.features.variants ? <Field label="Variant"><Select value={item.variantSku || ""} onChange={(event) => updateList('packages',index,{variantSku:event.target.value})}><option value="">Barcha variant</option>{form.variants.filter((variant) => variant.status !== "INACTIVE").map((variant) => <option key={variant.id || variant.sku} value={variant.sku}>{variant.name}</option>)}</Select></Field> : null}<Field label="Ichidagi soni"><input className="qp-input" type="number" min="0.001" step="0.001" placeholder="12" value={item.conversion} onChange={(event) => updateList('packages',index,{conversion:event.target.value})}/></Field><Field label="Qadoq tannarxi"><input className="qp-input" type="number" min="0" placeholder="0" value={item.costPrice} onChange={(event) => updateList('packages',index,{costPrice:event.target.value})}/></Field><Field label={`${activePriceLists.find((list) => list.isDefault)?.name || "Asosiy"} narxi`}><input className="qp-input" type="number" min="0" placeholder="0" value={item.price} onChange={(event) => updateList('packages',index,{price:event.target.value})}/></Field><Field label="Qadoq barcode"><input className="qp-input" placeholder="Barcode" value={item.barcode} onChange={(event) => updateList('packages',index,{barcode:event.target.value})}/></Field></div></div>)}
              </div> : null}
            ...
            <section className="qp-product-form-card">
              ...
              {(form.features.serial || form.features.lot || form.features.expiry) ? <div className="qp-tracking-notice"><CheckCircle2 size={18}/><div><strong>Kuzatuv talab qiladigan mahsulot</strong><span>{form.features.serial ? "Serial/IMEI " : ""}{form.features.lot ? "partiya " : ""}{form.features.expiry ? "va yaroqlilik muddati " : ""}ma’lumotlari mahsulotni Ombor → Kirim orqali qabul qilganda kiritiladi.</span></div></div> : null}
            </section>
