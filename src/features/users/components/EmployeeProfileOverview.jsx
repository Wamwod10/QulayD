...
function EmployeeProfileOverview({ db, employee }) {
  ...
  return <div className="qp-employee-profile">
    ...
    <div className="qp-employee-detail-grid"><section className="qp-card qp-employee-info-card"><div className="qp-card-head"><div><span>Profil</span><h3>Ish ma’lumotlari</h3></div><BriefcaseBusiness size={18}/></div><div className="qp-employee-info-list"><div><Phone size={16}/><span>Telefon</span><strong>{employee.phone || "—"}</strong></div><div><Building2 size={16}/><span>Filial</span><strong>{employee.branch || "—"}</strong></div><div><Warehouse size={16}/><span>Ombor</span><strong>{db.warehouses?.find((item)=>item.id===employee.warehouseId)?.name || "Biriktirilmagan"}</strong></div><div><MapPin size={16}/><span>Hudud</span><strong>{employee.territory || "Biriktirilmagan"}</strong></div></div></section>
